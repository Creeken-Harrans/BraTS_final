...
def build_training_transforms(
    ...
    foreground_labels: Optional[list[int] | tuple[int, ...]] = None,
    ...
    if use_mask_for_norm is not None and any(use_mask_for_norm):
        transforms.append(
            MaskImageTransform(
                apply_to_channels=[i for i, use_mask in enumerate(use_mask_for_norm) if use_mask],
...
def get_deep_supervision_scales(strides: list[list[int]] | tuple[tuple[int, ...], ...]):
    return list(list(item) for item in 1 / np.cumprod(np.vstack(strides), axis=0))[:-1]
